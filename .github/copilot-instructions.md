...
- **DNS CRUD**: Full create/edit/delete for all DNS record types via Cloudflare API. Routes: `record_add`, `record_edit`, `record_delete`. Records page uses modals for add/edit and confirmation modal for delete. Forms use hidden-input-before-checkbox pattern for boolean fields (e.g. `proxied`): hidden `value="0"` + checkbox `value="1"`. Backend must use `'1' in request.form.getlist('field')` (not `request.form.get()`) to correctly read the checkbox value.
